...
def test_max_iterations_default_when_unset():
    assert Player.max_iterations({}) == 25
    assert Player.max_iterations(None) == 25
...
def test_max_iterations_coerces_string_setting_strictly():
    assert Player.max_iterations({"max_iterations": "10"}) == 10
    with pytest.raises(ValueError):
        Player.max_iterations({"max_iterations": "not-a-number"})


def test_max_output_tokens_default_when_unset():
    assert Player.max_output_tokens({}) == 1024
    assert Player.max_output_tokens(None) == 1024
