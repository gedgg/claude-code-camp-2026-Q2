...
def make_fixture_step_recording_repl_calls(tmp_path, name="mud_fixture_step"):
    """A fixture step whose repl(**kwargs) records the kwargs it was called
    with to a JSON file, so the test (running in the same process, but with
    the fixture module exec'd as a fresh "boukensha" module) can inspect
    exactly what boukensha_loader passed through."""
    step_dir = tmp_path / name
    pkg_dir = step_dir / "src" / "boukensha"
    pkg_dir.mkdir(parents=True)
    record_path = tmp_path / f"{name}_repl_call.json"
    ...
    return step_dir, record_path


def test_mud_name_env_var_passes_legacy_mud_dict_and_disables_working_dir(monkeypatch, tmp_path):
    step_dir, record_path = make_fixture_step_recording_repl_calls(tmp_path)
    monkeypatch.setenv("BOUKENSHA_PATH", str(step_dir))
    monkeypatch.setenv("MUD_NAME", "Gandalf")
    monkeypatch.setenv("MUD_PASSWORD", "secret")
    monkeypatch.delenv("MUD_HOST", raising=False)
    monkeypatch.delenv("MUD_PORT", raising=False)
    ...
    assert recorded == {
        "working_dir": False,
        "mud": {"host": "localhost", "port": 4000, "name": "Gandalf", "password": "secret"},
    }


def test_mud_name_without_mud_password_aborts(monkeypatch, tmp_path):
    step_dir, _record_path = make_fixture_step_recording_repl_calls(tmp_path)
    monkeypatch.setenv("BOUKENSHA_PATH", str(step_dir))
    monkeypatch.setenv("MUD_NAME", "Gandalf")
    ...
    assert "MUD_NAME is set but MUD_PASSWORD is missing" in str(exc_info.value)


def test_no_mud_name_calls_repl_with_no_extra_kwargs(monkeypatch, tmp_path):
    step_dir, record_path = make_fixture_step_recording_repl_calls(tmp_path)
    monkeypatch.setenv("BOUKENSHA_PATH", str(step_dir))
    monkeypatch.delenv("MUD_NAME", raising=False)
...
def test_mud_host_and_port_env_vars_override_defaults(monkeypatch, tmp_path):
    step_dir, record_path = make_fixture_step_recording_repl_calls(tmp_path)
    monkeypatch.setenv("BOUKENSHA_PATH", str(step_dir))
    monkeypatch.setenv("MUD_NAME", "Gandalf")
    monkeypatch.setenv("MUD_PASSWORD", "secret")
    monkeypatch.setenv("MUD_HOST", "mud.example.com")
    monkeypatch.setenv("MUD_PORT", "5000")

    boukensha_loader.load_and_start_repl()

    recorded = json.loads(record_path.read_text())
    assert recorded["mud"] == {"host": "mud.example.com", "port": 5000, "name": "Gandalf", "password": "secret"}
