...
def make_repl(
    *, client=None, config_dir=None, provider="anthropic", model="m", version="0.10.0", api_key="sk-test", mud=None
):
    ctx = Context()
    logger = NullLogger()
    repl = Repl(
...
def test_banner_validation_restored_config_dir_missing(tmp_path):
    # 09_global_executable dropped this validation (a real, flagged
    # regression); this step restores it, confirming that regression really
    # was temporary — see this step's plan and 09's README.
    repl, _ctx, _logger = make_repl(config_dir=tmp_path / "nonexistent", api_key="sk-test")
    banner = repl._banner()

    assert "✗ directory not found" in banner


def test_banner_validation_restored_config_dir_present(tmp_path):
    repl, _ctx, _logger = make_repl(config_dir=tmp_path, api_key="sk-test")
    banner = repl._banner()

    assert "✗ directory not found" not in banner
    assert str(tmp_path) in banner


def test_banner_validation_restored_api_key_blank():
    repl, _ctx, _logger = make_repl(api_key="")
    assert "✗ API key not set" in repl._banner()


def test_banner_validation_restored_api_key_present():
    repl, _ctx, _logger = make_repl(api_key="sk-real")
    assert "✓ API key set" in repl._banner()


def test_banner_shows_mud_not_configured_when_mud_is_none():
    repl, _ctx, _logger = make_repl(mud=None)
    assert "mud:       (not configured)" in repl._banner()


def test_banner_shows_mud_status_via_probe(monkeypatch):
    repl, _ctx, _logger = make_repl(mud={"host": "example.test", "port": 4000, "name": "Gandalf"})
    monkeypatch.setattr(repl, "_probe_mud", lambda host, port, name: "(Reachable)")

    banner = repl._banner()

    assert "mud:       example.test:4000  (Reachable)" in banner


def test_probe_mud_reports_not_reachable_for_a_closed_port():
    # A real (loopback) TCP probe against a port nothing is listening on —
    # no mocking needed, this is a fast, self-contained real check.
    result = Repl._probe_mud("127.0.0.1", 1, None)
    assert result == "✗ not reachable"
