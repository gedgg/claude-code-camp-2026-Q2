...
MUD connection details come from settings.toml (mud.* keys) by default.
...
def load_and_start_repl() -> None:
    ...
    if os.environ.get("MUD_NAME"):
        ...
        if password is None:
            sys.exit("boukensha-py: MUD_NAME is set but MUD_PASSWORD is missing.")
    ...
    # automatically (via _mud_opts_from_config inside boukensha.repl).
