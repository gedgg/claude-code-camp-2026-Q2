...
NOTE: this module requires the separate `mud_manager` package (a Python
port of week0_explore/mud_manager — not yet ported; see this step's plan
in docs/plans/python_port/10_standard_tool_library for why that's treated
as its own, out-of-scope prerequisite). Unlike Ruby's `require "mud_manager"`
at the top of the file, the import here is deferred to inside register()
rather than sitting at module load time — otherwise merely importing
`boukensha` (which every other test in this port does) would fail whenever
mud_manager isn't installed, which is the common case until that separate
port exists.
"""
...
def register(registry, *, host: str = "localhost", port: int = 4000, name: str, password: str) -> None:
    import mud_manager

    session = mud_manager.Session(host=host, port=port)
    p = mud_manager.primitives

    def send_cmd(command: str) -> str:
        # Drain any stale buffered bytes (leftover login output, async
        # ticks, etc.) before sending so read_until_prompt sees only fresh
        # data produced by this command, then wait for CircleMUD's "> "
    ...
    def mud_connect() -> str:
        ...
        except mud_manager.Session.Error as e:
            return f"error: {e}"
    ...
    registry.tool(
        ...
        parameters={
            "kind": {
                "type": "string",
                "description": (
                    "What to check: score | inventory | equipment | gold | exits | time | weather "
