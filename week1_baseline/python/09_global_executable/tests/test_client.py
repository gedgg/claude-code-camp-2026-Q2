...
class FakeBuilder:
    ...
    def to_api_payload(self, *, max_output_tokens=1024, tools=None):
        self.payload_calls.append((max_output_tokens, tools))
        return {"model": "test-model", "max_output_tokens": max_output_tokens, "tools": tools}
...
def test_clean_200_returns_parsed_json(monkeypatch):
    ...
    assert builder.payload_calls == [(1024, None)]
...
def test_401_raises_friendlier_message_after_one_attempt_not_retried(monkeypatch, no_real_sleep):
    builder = FakeBuilder()
    attempts = []
    ...
    with pytest.raises(ApiError, match=r"authentication failed \(401\) — check your API key"):
        Client(builder).call()

    assert len(attempts) == 1  # 401 is not in the retryable-status set
    assert no_real_sleep == []
...
def test_headers_url_and_body_built_from_prompt_builder(monkeypatch):
    ...
    assert captured["body"] == {"model": "test-model", "max_output_tokens": 256, "tools": None}


def test_tools_kwarg_forwarded_to_prompt_builder(monkeypatch):
    builder = FakeBuilder()
    monkeypatch.setattr("boukensha.client.urllib.request.urlopen", lambda req: FakeResponse(200, b"{}"))
