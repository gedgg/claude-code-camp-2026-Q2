...
def test_banner_no_longer_validates_config_dir_or_api_key(tmp_path):
    # Simplified this step (matching Ruby's own history — see the
    # 09_global_executable plan's "known regressions" note): the banner just
    # prints the raw values, with no existence/blankness checks at all,
    # even when the config dir doesn't exist or the API key is blank.
    repl, _ctx, _logger = make_repl(config_dir=tmp_path / "nonexistent", api_key="")
    banner = repl._banner()

    assert "✗ directory not found" not in banner
    assert "✗ API key not set" not in banner
    assert "✓ API key set" not in banner
    assert str(tmp_path / "nonexistent") in banner


def test_banner_shows_config_provider_and_model_raw_values():
    repl, _ctx, _logger = make_repl(config_dir="/some/dir", provider="anthropic", model="claude-haiku-4-5")
    banner = repl._banner()

    assert "config:        /some/dir" in banner
    assert "provider:      anthropic" in banner
    assert "model:         claude-haiku-4-5" in banner


def test_banner_shows_default_placeholders_when_values_are_none():
    repl, _ctx, _logger = make_repl(config_dir=None, provider=None, model=None)
    banner = repl._banner()

    assert "config:        (default)" in banner
    assert "provider:      (default)" in banner
    assert "model:         (default)" in banner
