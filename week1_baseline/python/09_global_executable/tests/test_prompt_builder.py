...
def test_backend_is_a_public_attribute():
    # Needed since 06_the_logger: Logger#execution_metadata reads
    # builder.backend to compute provider/model/cost metadata.
    ctx = Context()
    backend = FakeBackend()
    builder = PromptBuilder(ctx, backend)
