...
def test_env_var_wins_regardless_of_cwd(tmp_path, monkeypatch):
    env_dir = tmp_path / "env_config"
    env_dir.mkdir()
    cwd_dir = tmp_path / "cwd_project"
    (cwd_dir / ".boukensha").mkdir(parents=True)

    monkeypatch.setenv("BOUKENSHA_DIR", str(env_dir))
    monkeypatch.chdir(cwd_dir)

    config = Config()
    assert config.dir == env_dir.resolve()


def test_cwd_boukensha_wins_over_home_default_when_env_unset(tmp_path, monkeypatch):
...
def test_falls_back_to_home_default_when_neither_env_nor_cwd_present(tmp_path, monkeypatch):
    monkeypatch.delenv("BOUKENSHA_DIR", raising=False)
