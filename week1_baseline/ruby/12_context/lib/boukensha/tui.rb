...
module Boukensha
  ...
  class Tui
    ...
    def sync_viewport
      @viewport.width  = @width
      @viewport.height = viewport_height
      @viewport.content = wrap(@conversation.join("\n"), @width)
      @viewport.goto_bottom
      @dirty = false
    end

    # Bubbles::Viewport only crops overlong lines horizontally (reachable via
    # scroll_left/right, which this Tui doesn't bind) — it never wraps. Word-wrap
    # here via lipgloss, which measures visible width (ANSI-escape aware) and
    # preserves existing blank lines / hard breaks.
    def wrap(text, width)
      return text if width <= 0

      Lipgloss::Style.new.width(width).render(text)
    end
